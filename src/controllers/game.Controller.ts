...
export class AuthController {
    ...
    try {
        ...
        [title, description, price, image, link, launch_date, active, stock_available, id_game]
        );
        ...
        await pool.query('DELETE FROM game_category WHERE id_game = ?', [id_game]);
        if (categories.length > 0) {
            const categoryValues = categories.map((id_category: number) => [id_game, id_category]);
            await pool.query('INSERT INTO game_category (id_game, id_category) VALUES ?', [categoryValues]);
        }
        }
